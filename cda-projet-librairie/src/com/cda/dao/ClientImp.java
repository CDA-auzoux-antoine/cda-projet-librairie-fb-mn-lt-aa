...
//		where isActive =false
		...
		try {
			...
			// premier id
			int rs = ps.executeUpdate();// exectution de la requete pour requete ecriture
	...
	public <E> Client save(E client) {
		if (c != null && client != null) {
			try {
				String request = "INSERT INTO client (id_client,nom_client,prenom_client,id_adresse_adresse,compte_login_compte,isActive ) VALUES (?,?,?,?,?,?)";
				PreparedStatement ps = null;
				ps = c.prepareStatement(request);
				ps.setInt(1, ((Client) client).getId());
				...
				ps.setString(5, ((Client) client).getCompte().getLogin());
